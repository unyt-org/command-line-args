...
export type TypeFromOptionType<T extends OptionType> = T extends "string"
  ? string
...
export type OptionValue<C extends OptionConfig> = C extends
  OptionConfig<infer T> ? _OptionsValue<
    C & (C["type"] extends string ? unknown : { type: "boolean" })
  >
  : never;

type hasArrayValue<C extends OptionConfig> = C["multiple"] extends true ? true
  : false;

type _OptionsValue<C extends OptionConfig> = C extends OptionConfig<infer T>
  ? hasArrayValue<C> extends true ? TypeFromOptionType<T>[]
  :
    | TypeFromOptionType<T>
    | (C["required"] extends true ? never
      : (hasArrayValue<C> extends true ? never
        : (C["default"] extends OptionConfig["default"] ? never : undefined)))
  : never;

export type OptionsConfig = { [name: string]: OptionConfig | undefined };
export type OptionsConfigValues<C extends OptionsConfig | undefined> = {
  [K in keyof C]: OptionValue<C[K] extends OptionConfig ? C[K] : OptionConfig>;
};
